//! Recall + speed harness for the [`RabitqExtQuantizer`].
//!
//! Extended-RaBitQ-encodes a dense f32 dataset (rotated multi-bit document codes with
//! per-document scan metadata), runs exhaustive search for a set of queries, and reports:
...
//!     reranker over the top-`k'` candidates could reach; and
...
use vectorium::{
    Dataset, DenseDataset, DenseVectorEncoder, FlatIndex, Index, PlainDenseDataset,
    RabitqExtConfig, RabitqExtQuantizer, SpaceUsage, VectorEncoder, VectorId,
};

#[derive(Parser, Debug)]
#[clap(author, version, about = "Evaluate the extended RaBitQ encoder: recall vs. groundtruth + search speed", long_about = None)]
struct Args {
    ...
    /// Quantize each document to this many bits per component (1 sign bit +
    /// `total_bits - 1` magnitude bits), in 2..=9. For the plain 1-bit sign code use the
    /// `evaluate_rabitq` binary (the RabitqQuantizer encoder) instead.
...
fn main() {
    ...
    // --- Extended-RaBitQ-encode the documents ----------------------------------
    ...
    match args.metric {
        Metric::L2 => run_rabitq_ext!(SquaredEuclideanDistance, "l2"),
        Metric::Ip => run_rabitq_ext!(DotProduct, "ip"),
    }
}

/// Report config, run the parallel search, and print the recall@k table for `dataset`.
///
/// Generic over the encoder's metric; extended RaBitQ stores multi-bit `u64` plane-major
/// documents in every case and the metric only decides the scoring/ordering. Recall only reads
/// the *ranking*, so the metric's ordering (maximize for [`DotProduct`], minimize for squared
/// Euclidean) is handled by the distance's own `Ord` and never surfaces here.
fn run<E>(
