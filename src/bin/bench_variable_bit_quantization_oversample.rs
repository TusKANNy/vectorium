...
#[clap(
    author,
    version,
    about = "Benchmark variable-bit uniform sparse quantization (1..=8 bits) with oversampling: \
             quantized search returns k' candidates, recall@k counts how many of the f32 top-k \
             land anywhere in that k'-candidate set (no f32 rerank — assumes a downstream \
             reranker can refine the deeper list)."
)]
struct Args {
    ...
    #[clap(short, long, default_value = "variable_bit_quantization_oversample_results.tsv")]
    output: String,
...
fn main() {
    ...
    for nbits in [1, 2, 3, 4, 5, 6, 7, 8] {
        println!(
            "\n=== uniform quantization, nbits={nbits}, k_prime={} ===",
            args.k_prime
        );
    ...
    println!("\n=== Summary (k={}, k_prime={}) ===", args.k, args.k_prime);
    println!(
        "{:<10} {:>10} {:>12} {:>12} {:>12} {:>10}",
    ...
    writeln!(
        f,
        "nbits\tk\tk_prime\tsize_gib\ttrain_time_s\tbuild_time_s\tsearch_time_s\trecall_at_k"
    )
    .unwrap();
