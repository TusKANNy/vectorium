//! Single-thread search-efficiency benchmark for the 4-bit *clustered-centroid*
//! sparse quantizer ([`PackedClusteredCentroidSparseQuantizer`]). Doc bytes are
//! the same 4-bit interleaved layout as the f32-LUT and f16-LUT variants; the
//! difference is the codebook is shared across components grouped by k-means
//! over the per-component centroid rows, so the per-query LUT is replaced by
//! three small L2-resident tables (densified `q`, `cluster_of`, shared codebook).
//! See [`packed_centroid_based_quantization_sparse_scalar`] for details.
//!
//! Same CLI surface as `bench_4bit_efficiency_kmeans` plus `--n-clusters K`,
//! `--cluster-iters`, and `--cluster-redo` for the second-stage k-means.
...
use vectorium::encoders::packed_centroid_based_quantization_sparse_scalar::PackedClusteredCentroidSparseQuantizer;
...
#[clap(
    author,
    version,
    about = "4-bit clustered-centroid sparse quantization search efficiency (single-thread)"
)]
struct Args {
    #[clap(short, long)]
    input_file: String,
    ...
    /// Per-component (stage-1) Lloyd iters.
    #[clap(long, default_value_t = 10)]
    n_iterations: usize,

    /// Number of clusters K used to group the dim×16 per-component centroid rows.
    #[clap(long, default_value_t = 1024)]
    n_clusters: usize,

    /// Lloyd iters for the cluster-stage k-means over the rows.
    #[clap(long, default_value_t = 25)]
    cluster_iters: usize,

    /// Redos for the cluster-stage k-means.
    #[clap(long, default_value_t = 1)]
    cluster_redo: usize,

    #[clap(short, long, default_value = "kmeans_4bit_clustered_efficiency.tsv")]
    output: String,
...
fn time_search_serial(
    dataset: &PackedSparseDataset<PackedClusteredCentroidSparseQuantizer>,
...
fn main() {
    ...
    println!(
        "Repeats: {} | single-threaded | stage-1 Lloyd iters: {} | n_clusters: {} | \
         cluster-stage iters: {} (redo {})",
        args.repeats,
        args.n_iterations,
        args.n_clusters,
        args.cluster_iters,
    ...
    println!(
        "\n=== Training PackedClusteredCentroidSparseQuantizer @ nbits={NBITS} (K={}) ===",
        args.n_clusters
    );
    ...
    let quantizer = PackedClusteredCentroidSparseQuantizer::train(
        &training_data,
    ...
    let mut growable: PackedSparseDatasetGrowable<PackedClusteredCentroidSparseQuantizer> =
        PackedSparseDatasetGrowable::new(quantizer);
    for vec in dataset_f32.iter() {
        growable.push(vec);
    }
    let dataset_quantized: PackedSparseDataset<PackedClusteredCentroidSparseQuantizer> =
        growable.into();
    ...
    println!(
        "\n=== Summary (best of {} repeats, single-thread, clustered K={}) ===",
        args.repeats, args.n_clusters
    );
    ...
    writeln!(
        f,
        "encoder\tnbits\tn_clusters\tn_iterations\tcluster_iters\tcluster_redo\t\
         n_docs\tdim\tnnz\tindex_size_gib\ttrain_time_s\tbuild_time_s\t\
         n_queries\twarmup\trepeats\tk\ttotal_s\tmean_us\tqps\trecall_at_k"
    )
    .unwrap();
    writeln!(
        f,
        "kmeans_4bit_clustered\t{NBITS}\t{nc}\t{niter}\t{ci}\t{cr}\t\
         {n}\t{dim}\t{nnz}\t{size_gib:.6}\t{train_time:.6}\t{build_time:.6}\t\
         {nq}\t{warmup}\t{rep}\t{k}\t{best_total:.6}\t{mean_us:.3}\t{qps:.3}\t{recall:.6}",
        nc = args.n_clusters,
        niter = args.n_iterations,
        ci = args.cluster_iters,
        cr = args.cluster_redo,
        nq = n_queries,
        rep = args.repeats,
        k = args.k,
    )
