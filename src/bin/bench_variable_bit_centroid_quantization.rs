...
#[clap(
    author,
    version,
    about = "Benchmark variable-bit centroid-based sparse quantization (1..=8 bits). \
             Per-iteration Lloyd diagnostics are printed by the trainer."
)]
struct Args {
    ...
    #[clap(short, long, default_value = "variable_bit_centroid_quantization_results.tsv")]
    output: String,
...
fn main() {
    ...
    for nbits in [1u8, 2, 3, 4, 5, 6, 7, 8] {
        ...
        // `print_kmeans_iter_summary` — total WCSS, mean WCSS/value, % reassignments,
        // mean centroid drift, and #dims with empty clusters per Lloyd pass.
        let start = Instant::now();
    ...
    println!("\n=== Summary (n_iterations={}) ===", args.n_iterations);
    println!(
        "{:<10} {:>10} {:>12} {:>12} {:>12} {:>10}",
    ...
    writeln!(
        f,
        "nbits\tn_iterations\tsize_gib\ttrain_time_s\tbuild_time_s\tsearch_time_s\trecall_at_k"
    )
    .unwrap();
