//! Single-thread search-efficiency benchmark for the specialised 4-bit *centroid*
//! (k-means) sparse quantization kernel (`PackedCentroidSparseQuantizer`,
//! hardcoded `nbits = 4`, dispatching to the `dot_product_centroid_nbits4`
//! SIMD-gather path with the interleaved nibble layout).
//!
...
//!
//! Per-iteration Lloyd diagnostics (WCSS / reassignments / drift / empty
//! clusters) are emitted by the trainer itself; this bin doesn't add any.
...
#[clap(
    author,
    version,
    about = "4-bit centroid (k-means) sparse quantization search efficiency (single-thread)"
)]
struct Args {
    ...
    #[clap(short, long, default_value = "kmeans_4bit_efficiency.tsv")]
    output: String,
...
fn time_search_serial(
    dataset: &PackedSparseDataset<PackedCentroidSparseQuantizer>,
...
fn main() {
    ...
    println!(
        "Repeats: {} | single-threaded | Lloyd iters: {}",
        args.repeats, args.n_iterations
    );

    // ── Train and build packed 4-bit centroid dataset ──────────────
    println!(
    ...
    let mut growable: PackedSparseDatasetGrowable<PackedCentroidSparseQuantizer> =
        PackedSparseDatasetGrowable::new(quantizer);
    for vec in dataset_f32.iter() {
        growable.push(vec);
    }
    let dataset_quantized: PackedSparseDataset<PackedCentroidSparseQuantizer> = growable.into();
    ...
    writeln!(
        f,
        "kmeans_4bit\t{NBITS}\t{niter}\t{n}\t{dim}\t{nnz}\t{size_gib:.6}\t{train_time:.6}\t{build_time:.6}\t\
