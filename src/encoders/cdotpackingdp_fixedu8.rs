...
pub struct CDotPackingDpFixedu8Encoder<const MAX_LEN: usize = 16> {
    dim: usize,
    num_clusters: usize,
    all_refs: Vec<u16>,
    max_ref_size: usize,
...
impl<const MAX_LEN: usize> PartialEq for CDotPackingDpFixedu8Encoder<MAX_LEN> {
...
impl<const MAX_LEN: usize> sealed::Sealed for CDotPackingDpFixedu8Encoder<MAX_LEN> {}
...
struct CDotPackingDpFixedu8<'a, const MAX_LEN: usize> {
    map_n: usize,
    res_n: usize,
    ref_id: u16,
    selectors: &'a [u8],
...
impl<'a, const MAX_LEN: usize> CDotPackingDpFixedu8<'a, MAX_LEN> {
    unsafe fn from_unchecked_slice(slice: &'a [u64]) -> Self {
        let bytes = unsafe { try_cast_slice::<u64, u8>(slice).unwrap_unchecked() };
    ...
    #[inline(always)]
    fn mapped_iter(&self) -> DotPackingDpIter<'a, MAX_LEN> {
        DotPackingDpIter {
            n: self.map_n,
            processed_count: 0,
            sel_idx: 0,
...
impl<const MAX_LEN: usize> SparseDataEncoder for CDotPackingDpFixedu8Encoder<MAX_LEN> {
    ...
    fn decode_vector<'a>(
        ...
        let view = unsafe { CDotPackingDpFixedu8::<MAX_LEN>::from_unchecked_slice(encoded.data()) };
        let ref_indices = self.get_ref_slice(view.ref_id);

        let mut mapped_comps = Vec::with_capacity(view.map_n);
        let mut mapped_vals = Vec::with_capacity(view.map_n);
        let mut res_comps = Vec::with_capacity(view.res_n);
        let mut res_vals = Vec::with_capacity(view.res_n);
...
impl<const MAX_LEN: usize> PackedSparseVectorEncoder for CDotPackingDpFixedu8Encoder<MAX_LEN> {
...
impl<const MAX_LEN: usize> VectorEncoder for CDotPackingDpFixedu8Encoder<MAX_LEN> {
    ...
    type Evaluator<'e>
        = CDotPackingDpFixedu8QueryEvaluator<'e, MAX_LEN>
    where
        Self: 'e;
    fn query_evaluator<'e>(&'e self, query: Self::QueryVector<'_>) -> Self::Evaluator<'e> {
        CDotPackingDpFixedu8QueryEvaluator::new(query, self)
    }
    fn vector_evaluator<'e, 'v>(&'e self, vector: Self::EncodedVector<'v>) -> Self::Evaluator<'e> {
        ...
        CDotPackingDpFixedu8QueryEvaluator::<MAX_LEN>::new(decoded.as_view(), self)
    }
...
pub struct CDotPackingDpFixedu8QueryEvaluator<'a, const MAX_LEN: usize = 16> {
    encoder: &'a CDotPackingDpFixedu8Encoder<MAX_LEN>,
    dense_query: Vec<f32>,
}

impl<'a, const MAX_LEN: usize> CDotPackingDpFixedu8QueryEvaluator<'a, MAX_LEN> {
    pub fn new(
    ...
    unsafe fn simd_compute_distance(&self, vector: PackedVectorView<'_, u64>) -> DotProduct {
        let view = unsafe { CDotPackingDpFixedu8::<MAX_LEN>::from_unchecked_slice(vector.data()) };
...
impl<'a, 'v, const MAX_LEN: usize> QueryEvaluator<PackedVectorView<'v, u64>>
    for CDotPackingDpFixedu8QueryEvaluator<'a, MAX_LEN>
{
    type Distance = DotProduct;
...
impl<const MAX_LEN: usize> SpaceUsage for CDotPackingDpFixedu8Encoder<MAX_LEN> {
    fn space_usage_bytes(&self) -> usize {
        ...
        self.all_refs.len() * 2 + size_of_mapping + 16
    }
}
