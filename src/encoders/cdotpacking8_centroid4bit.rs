...
impl<'a> NibbleReader<'a> {
    ...
    /// Expand the next full block (8 codes) to `Simd<u8, 8>` in natural lane order
    /// `[v0..v7]` (no cross-lane shuffle), ready to be widened into gather indices:
    /// `dup=[b0..b3,b0..b3] >> [0,0,0,0,4,4,4,4] & 0xF`.
    #[inline(always)]
    fn next_block_codes(&mut self) -> Simd<u8, N> {
...
// ---------------------------------------------------------------------------
// 4-bit per-component centroid value quantizer.
// ---------------------------------------------------------------------------

/// Per-component centroid quantizer producing 4-bit codes (`0..=15`). `centroids` is a
/// flat `dim·16` table; `centroids[c·16 + v]` is the v-th centroid for component `c`, with
/// each per-component row sorted ascending so [`Self::encode_value`] can binary-search.
...
impl Centroid4BitQuantizer {
    ...
    /// Nearest-centroid index for `value` in component `component` (binary search over the
    /// ascending per-component row). Mirrors `PackedCentroidSparseQuantizer::quantize`.
    #[inline(always)]
