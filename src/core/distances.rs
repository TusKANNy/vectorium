...
/// exactly once (into `d_buf`), then the decoded f32 slice is reused for all query-token
/// dot products. This keeps the query matrix (small, repeatedly accessed) hot in L1 cache
/// and minimises conversion work — a Q× reduction compared to an outer-query loop.
///
/// # Arguments
